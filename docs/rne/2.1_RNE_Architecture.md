...
2. **Acquisition quotidienne des flux**
   ...
   - Le DAG `get_flux_rne` télécharge non seulement le flux du jour, mais aussi tous les flux manquants entre la dernière date connue et le jour en cours.
   - Le point de départ est déterminé à partir de latest_rne_date.json s’il existe, sinon par la date de repli RNE_DEFAULT_START_DATE.
   - Les flux sont téléchargés et déposés dans MinIO rne/flux/.
   - Le flux le plus récent (jour J) est volontairement ignoré lors de la consolidation, la base produite couvre donc les données jusqu’à J–1.
...
| **latest_rne_date.json** | Contient la date de reprise pour le prochain run (dernier flux traité + 1 jour), pas la date du flux lui-même. | `rne/database/` |
